...
if (idCkBoolean == true) { //아이디 중복검사에서 통과하면
	...
	response.sendRedirect(request.getContextPath() + "/addCustomer.jsp?idCkBoolean="+idCkBoolean+"&checkedId=" + id + "&idCkMsg="+idCkMsg);
} else {
	idCkMsg="This id is already in use.";
	response.sendRedirect(request.getContextPath() + "/addCustomer.jsp?idCkBoolean="+idCkBoolean+"&idCkMsg="+idCkMsg);
}
%>
